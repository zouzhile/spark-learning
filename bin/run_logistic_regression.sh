...
trainingData=/user/zouzhile/lr/data/training.txt
testData=/user/zouzhile/lr/data/test.txt
numIterations=100
stepSize=1
miniBatchFraction=1.0
regParam=0.1
output=/user/zouzhile/lr/output

hadoop fs -rm -r -f -skipTrash $output

$SPARK_HOME/bin/spark-submit --driver-class-path $SPARK_CLASSPATH --queue=apg_p7 --class LogisticRegressionDriver --master yarn --deploy-mode cluster $PWD/target/spark-examples-1.2.jar $trainingData $testData $numIterations $stepSize $miniBatchFraction $regParam $output
